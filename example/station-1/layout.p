...
fof(node_v2, axiom, (
   (![X]: ![T]: (at(succ(X), T, v2) <=> ((at(X, T, v2) & ~goes(X, T)) | (at(X, T, in2) & goes(X, T) & open(X, in2)))))
)).
% transition axiom for output node out1
fof(node_out1, axiom, (
   (![X]: ![T]: (at(succ(X), T, out1) <=> ((at(X, T, v1) & goes(X, T) & (switch(X, v1) = out1)) | (at(X, T, v2) & goes(X, T) & (switch(X, v2) = out1)))))
)).
% transition axiom for output node out2
fof(node_out2, axiom, (
   (![X]: ![T]: (at(succ(X), T, out2) <=> ((at(X, T, v1) & goes(X, T) & (switch(X, v1) = out2)) | (at(X, T, v2) & goes(X, T) & (switch(X, v2) = out2)))))
)).
...
fof(switch_v2, axiom, (
   (![X]: ((switch(X, v2) = out1) | (switch(X, v2) = out2)))
)).
